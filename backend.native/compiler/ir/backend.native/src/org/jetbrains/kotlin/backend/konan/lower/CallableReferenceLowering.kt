...
internal class CallableReferenceLowering(val context: Context): FileLoweringPass {
    ...
    private inner class FunctionReferenceBuilder(val parent: IrDeclarationParent,
        ...
        fun build(): BuiltFunctionReference {
            ...
            when {
                isKFunction -> {
                    superTypes += kFunctionImplSymbol.typeWith(referencedFunction.returnType)
                    functionClass = symbols.kFunctions[numberOfParameters].owner
                ...
                isKSuspendFunction -> {
                    ...
                    functionClass = symbols.functions[numberOfParameters].owner
                    superTypes += functionClass.typeWith(functionParameterTypes + referencedFunction.returnType)
                    // Anonymous functions and lambdas never inherit SuspendFunctionX.
                    suspendFunctionClass = null
                }
            }
        ...
        private fun buildInvokeMethod(superFunction: IrSimpleFunction) = WrappedSimpleFunctionDescriptor().let {
            IrFunctionImpl(
                    ...
                    Visibilities.PRIVATE,
                    Modality.FINAL,
                    if (referencedFunction.isSuspend && !superFunction.isSuspend)
                        irBuiltIns.anyNType
                    else referencedFunction.returnType,
                    isInline = false,
                    isExternal = false,
                    isTailrec = false,
